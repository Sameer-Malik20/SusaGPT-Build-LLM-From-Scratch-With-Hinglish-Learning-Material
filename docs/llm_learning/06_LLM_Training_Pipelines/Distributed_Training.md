# 🛳️ Distributed Training Deep Dive — Bade LLMs ko train karna (Scaling Mastery)
> **Level:** Advanced → Expert | **Language:** Hinglish | **Goal:** DP, DDP, FSDP, Pipeline, aur Tensor Parallelism master karo.

---

## 📋 Table of Contents: Ek GPU se aage AI scaling

| Strategy | Full Name | Kaise Kaam Karta Hai? |
|----------|-----------|-----------------------|
| **1. DP** | Data Parallelism | Model ko saare GPUs par copy karo, data split karo. |
| **2. DDP** | Distributed Data Par. | Gradients ko efficiently sync karo (AllReduce). |
| **3. FSDP** | Fully Sharded Data Par. | Model, gradients, aur optimizer ko shard karo. |
| **4. TP** | Tensor Parallelism | Ek single matrix multiplication ko split karo. |
| **5. PP** | Pipeline Parallelism | Model layers ko assembly line ki tarah split karo. |
| **6. ZeRO** | Zero Redundancy Opt. | DeepSpeed ka magic (Stage 1, 2, 3). |
...
### A. Data Parallelism (DP) - Purana Tareeka
...
### B. Distributed Data Parallelism (DDP) - Industry Standard
...
## 2. 🧊 FSDP (Fully Sharded Data Parallelism): Memory Bachane Wala
...
## 🚀 DeepSpeed aur ZeRO (Zero Redundancy Optimizer)
...
- [ ] Tensor Parallelism kab use hota hai? (Bade layers).
- [ ] ZeRO Stage 3 kya save karta hai? (Full Memory).

> **Expert Tip:** Distributed training karna utna hi **Networking (NVLink/InfiniBand)** ke baare mein hai jitna **GPU Compute** ke baare mein. Speed bottleneck aksar bandwidth hi hota hai.