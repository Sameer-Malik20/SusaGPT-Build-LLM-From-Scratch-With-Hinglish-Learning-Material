...
class LoRAParameterizedLinear(nn.Module):
    # Custom parameter-efficient LoRA adapter layer.
    # W_new = W_old + (B @ A) * (alpha / r)
    # W_old is frozen, only A and B are trained.
    ...
    def reset_parameters(self):
        # Initialize A with Kaiming uniform and B with zeros so delta is initially 0
...
def inject_lora(model, r=8, alpha=16, dropout=0.05, target_modules=None):
    if target_modules is None:
        target_modules = ["W_q", "W_v", "w1", "w3"]
    ...
    for name, module in list(model.named_modules()):
        ...
        if is_target and isinstance(module, nn.Linear):
            ...
            parent = get_submodule(model, parent_name)
            
            lora_layer = LoRAParameterizedLinear(module, r=r, alpha=alpha, dropout=dropout)
            setattr(parent, child_name, lora_layer)
            replaced_count += 1
            
    print(f"LoRA injected into {replaced_count} layers (r={r}, alpha={alpha})")


def merge_and_unload_lora(model):
    replaced_count = 0
    for name, module in list(model.named_modules()):
        if isinstance(module, LoRAParameterizedLinear):
            with torch.no_grad():
                delta_weight = (module.lora_B @ module.lora_A) * module.scaling
                module.linear.weight.copy_(module.linear.weight + delta_weight)
    ...
    print(f"LoRA merged and unloaded from {replaced_count} layers")
